...
#
# This script exercises installation plus every intent command against a real
# project.
...
bootstrap_lib <- file.path(sandbox_dir, "bootstrap-library")
...
# ── 1. install intent into a bootstrap library ──────────────────────────
cat("\n=== 1. install intent ===\n")
install.packages(
  c("callr", "desc", "jsonlite", "pak", "renv"),
  lib = bootstrap_lib,
...
library(intent, lib.loc = bootstrap_lib)
stopifnot(requireNamespace("intent", lib.loc = bootstrap_lib, quietly = TRUE))
cat("✔ intent installed into bootstrap library\n")
...
stopifnot(dir.exists(file.path(project_library, "intent")))
lock <- renv::lockfile_read(file.path(project_dir, "renv.lock"))
stopifnot("intent" %in% names(lock$Packages))
cat("✔ init hydrated intent into the project library and lockfile\n")

restart_has_intent <- callr::r(
  function(project) {
    Sys.unsetenv("RENV_PROJECT")
    setwd(project)
    source(file.path(project, "renv/activate.R"))
    requireNamespace("intent", quietly = TRUE)
  },
  args = list(project_dir),
  libpath = bootstrap_lib,
  user_profile = FALSE,
  system_profile = FALSE
)
stopifnot(isTRUE(restart_has_intent))
cat("✔ intent is available after renv activation in a fresh R session\n")

# ── 3. status (empty project) ───────────────────────────────────────────
cat("\n=== 3. status (empty project) ===\n")
...
json_plan <- as.character(intent::add(
  "cli",
  project = project_dir,
  dry_run = TRUE
))
...
cat("\n=== 13. sync prune ===\n")
lock <- renv::lockfile_read(file.path(project_dir, "renv.lock"))
lock$Packages$orphan_test <- list(
  Package = "orphan_test",
  Version = "1.0.0",
  Source = "Repository",
  Repository = "RSPM"
...
cat("\n═══════════════════════════════════════════════════\n")
cat("All 13 end-to-end checks passed.\n")
cat("═══════════════════════════════════════════════════\n")
